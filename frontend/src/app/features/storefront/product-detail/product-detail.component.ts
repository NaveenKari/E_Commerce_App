      ...
      <div class="mx-auto max-w-5xl px-4 py-10">
        ...
        <div class="mt-6 grid grid-cols-1 gap-10 md:grid-cols-2">
          <div class="relative aspect-square overflow-hidden rounded-2xl bg-neutral-100">
            <img [src]="resolveImageUrl(product()!.image)" [alt]="product()!.productName" class="h-full w-full object-cover" />
          ...
          <div class="flex flex-col gap-4">
            <div class="flex items-start justify-between gap-4">
              ...
              <button
                type="button"
                class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-neutral-200 text-neutral-500 transition hover:text-primary-600"
                [class.text-primary-600]="isSaved"
                [class.border-primary-200]="isSaved"
                (click)="onToggleWishlist()"
                [attr.aria-label]="isSaved ? 'Remove from wishlist' : 'Save to wishlist'"
              >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" [attr.fill]="isSaved ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="2">
...
export class ProductDetailComponent implements OnInit {
  ...
  get isSaved(): boolean {
    ...
    return product ? this.wishlistService.isSaved(product.productId) : false;
  }
  ...
  onToggleWishlist(): void {
    ...
    const request$ = this.isSaved ? this.wishlistService.remove(product.productId) : this.wishlistService.add(product.productId);
